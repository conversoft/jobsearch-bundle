...
$GLOBALS['TL_DCA']['tl_dmajobsearch'] = [
    // Config
    'config' => [
        'dataContainer' => 'Table',
	    ...
	    'onload_callback' => [
	        ['tl_dmajobsearch', 'myLoadCallback']
	    ],
	    'onsubmit_callback' => [
	        ['tl_dmajobsearch', 'mySaveCallback']
	    ]
    ],
    ...
    'fields' => [
        ...
        'title' => [
            'label' => &$GLOBALS['TL_LANG']['tl_dmajobsearch']['title'],
            'exclude' => true,
            'search' => true,
            'sorting' => true,
            'flag' => 1,
...
// use  Dma\Model\DmajobsearchModel;

class tl_dmajobsearch extends \Backend
{
    /**
     * Constructor
     *
    ...
    public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        if (!$this->User->hasAccess('tl_dmajobsearch::published', 'alexf')) {
            return '';
        }
    ...
    public function toggleVisibility($intId, $blnVisible, DataContainer $dc = null)
    {
        ...
        if ($dc) {
            ...
            // $objModel = DmajobsearchModel::findById($intId);
        }
...
//        $this->checkPermission();
        ...
        $objVersions = new Versions('tl_dmajobsearch', $intId);
        $objVersions->initialize();

        // Trigger the save_callback
        if (is_array($GLOBALS['TL_DCA']['tl_dmajobsearch']['fields']['published']['save_callback'])) {
            foreach ($GLOBALS['TL_DCA']['tl_dmajobsearch']['fields']['published']['save_callback'] as $callback) {
        ...
        // Update the database
        Database::getInstance()->prepare("UPDATE tl_dmajobsearch SET tstamp=" . time() . ", published='" . ($blnVisible ? '1' : '') . "' WHERE id=?")
            ->execute($intId);
